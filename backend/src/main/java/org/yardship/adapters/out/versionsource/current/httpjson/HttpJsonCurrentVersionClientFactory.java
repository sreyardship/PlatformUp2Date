...
/**
 * Builds a ready {@link HttpJsonCurrentVersionClient} for a given base URL — the only Arc-bound piece left
 * after {@code HttpJsonCurrentSource} became a pure POJO. It is the sole boundary that knows how to
 * construct the transport for the {@code http-json} current-version kind: it owns the
 ...
 * {@link RedirectFollowingHttpJsonCurrentVersionTransport} for the transport itself.
 *
 ...
 * to keep the JVM default trust bundle. The {@code HttpJsonCurrentSourceFactory} is responsible for
 ...
 * {@code trustStore} at the caller ({@code HttpJsonCurrentSourceFactory}) level; this boundary does not
 * itself enforce that.
 */
@ApplicationScoped
public class HttpJsonCurrentVersionClientFactory {
    ...
    public HttpJsonCurrentVersionClient build(
        ...
        RedirectFollowingHttpJsonCurrentVersionTransport transport = new RedirectFollowingHttpJsonCurrentVersionTransport(
                url, authFilter, trustStore, insecureSkipTlsVerify);
        // A method reference, not a class `implements HttpJsonCurrentVersionClient` — that interface
