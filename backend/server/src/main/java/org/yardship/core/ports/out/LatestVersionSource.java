 ...
 * concerns owned by the factory that constructs it. The auth concern (e.g. a GitHub token) is
 * also owned by the concrete source/factory, never by this port. Per-app failure isolation lives
 * in the scrape loop, so an implementation is free to throw on an unreachable or unparseable
 * upstream.
 */
